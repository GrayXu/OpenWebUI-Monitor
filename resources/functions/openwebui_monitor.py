...
class Filter:
    ...
    def _prepare_user_dict(self, user: dict) -> dict:
        """将 user 对象转换为可序列化的字典"""
        user_dict = dict(user)
        if "valves" in user_dict and hasattr(user_dict["valves"], "model_dump"):
            user_dict["valves"] = user_dict["valves"].model_dump()  # Pydantic v2
        elif "valves" in user_dict and hasattr(user_dict["valves"], "dict"):
            user_dict["valves"] = user_dict["valves"].dict()  # Pydantic v1
        return user_dict
    ...
    async def outlet(self, body: dict, __metadata__: Optional[dict] = None, __user__: Optional[dict] = None, __event_emitter__: Optional[callable] = None) -> dict:
        ...
        try:
            ...
            if self.valves.language == "en":
                ...
                if self.valves.show_cost:
                    stats_list.append(self.get_text("cost", cost=response_data["totalCost"]*7.2))
                if self.valves.show_balance:
                    stats_list.append(self.get_text("balance", balance=response_data["newBalance"]*7.2))
